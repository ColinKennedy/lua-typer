  ...
  for _, binding in ipairs(model.bindings) do
    ...
    if reason and not binding.class_tag then
      ...
      if not typed_as_class then
        ctx.emit(diagnostic.new(model.path, binding, "missing-class",
    ...
    if binding.class_tag and binding.class_tag.name then
      ...
      for field_name, entry in pairs(binding.fields) do
        local declared = entry.annotated
          or (decl and registry_mod.has_field(registry, decl, field_name))
        if not declared then
          ctx.emit(diagnostic.new(model.path, entry, "missing-field",
            ("field '%s' is assigned on class '%s' but has no ---@field")
              :format(field_name, binding.class_tag.name),
      ...
      if ctx.config.require_method_fields then
        for method_name, entry in pairs(binding.methods) do
          ...
          if not declared then
            ctx.emit(diagnostic.new(model.path, entry, "missing-field",
              ("method '%s' has no ---@field on class '%s'")
                :format(method_name, binding.class_tag.name),
      ...
      if binding.inherits then
        ...
        if base_name and #declared_parents == 0 then
          ctx.emit(diagnostic.new(model.path,
            { l = binding.class_tag.l, c = binding.class_tag.name_col or binding.class_tag.c,
              ec = binding.class_tag.name_ec },
            "missing-inherit",
            ("class '%s' inherits from '%s' at runtime but does not declare it")
              :format(binding.class_tag.name, base_name),
