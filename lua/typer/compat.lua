...
function M.bytecode_tag()
  local probe = string.dump(function() return 1 end)
...
function M.normalize(path)
  ...
  if absolute then joined = "/" .. joined end
...
function M.join(...)
  ...
  for i = 1, #parts do
    local piece = parts[i]
    if piece and piece ~= "" then out[#out + 1] = (piece:gsub("/+$", "")) end
  end
