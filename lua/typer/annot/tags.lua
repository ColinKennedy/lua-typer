...
local KNOWN = {
  class = true, field = true, param = true, ["return"] = true, type = true,
  alias = true, enum = true, generic = true, overload = true, meta = true,
  vararg = true, cast = true, operator = true, module = true,
  private = true, protected = true, package = true, ["public"] = true,
  nodiscard = true, async = true, deprecated = true, see = true,
  diagnostic = true, source = true, version = true, as = true,
}
...
local function parse_class(tag, text, comment)
  ...
  if not name then
    tag.err = { typer_error = true, kind = "tag", msg = "class name expected",
                l = comment.l, c = text_base_col(comment) + offset - 1 }
    return
  end
...
local function parse_field(tag, text, comment)
  ...
  if not scope then scope, scope_stop = match(text, "^%s*(private)%s+()") end
  if not scope then scope, scope_stop = match(text, "^%s*(protected)%s+()") end
  if not scope then scope, scope_stop = match(text, "^%s*(package)%s+()") end
  if scope then
    tag.scope = scope
    offset = scope_stop
  end
  ...
  local name = match(text, "^([%a_][%w_]*)", offset)
  if not name then
    tag.err = { typer_error = true, kind = "tag", msg = "field name expected",
                l = comment.l, c = text_base_col(comment) + offset - 1 }
    return
  end
...
local function parse_param(tag, text, comment)
  ...
  if sub(text, offset, offset + 2) == "..." then
    ...
    if not name then
      tag.err = { typer_error = true, kind = "tag", msg = "parameter name expected",
                  l = comment.l, c = text_base_col(comment) + offset - 1 }
      return
    end
    offset = offset + #name
...
local function parse_type_tag(tag, text, comment)
  ...
  repeat
    ...
    offset = next_offset
    local _, comma_stop = find(text, "^%s*,%s*", offset)
    if comma_stop then offset = comma_stop + 1 end
  until not comma_stop
  tag.type = tag.list[1]
end
...
local function parse_generic(tag, text)
  ...
  for piece in text:gmatch("[^,]+") do
    ...
    if name then tag.names[#tag.names + 1] = name end
  end
end
...
local function parse_alias(tag, text, comment)
  ...
  local name = match(text, "^([%a_][%w_%.]*)", offset)
  if not name then
    tag.err = { typer_error = true, kind = "tag", msg = "alias name expected",
                l = comment.l, c = text_base_col(comment) + offset - 1 }
    return
  end
...
function M.parse_comment(comment, in_alias)
  if not comment.doc then return nil end
  ...
  if continuation then
    ---@type typer.Tag
    local tag = {
      kind = "alias-item", l = comment.l, c = comment.c,
      text_col = text_base_col(comment), raw = continuation,
    }
  ...
  local ok, err = pcall(function()
    ...
    elseif name == "enum" then
      local enum_name = match(raw, "^%s*%(key%)%s*([%a_][%w_%.]*)")
        or match(raw, "^%s*([%a_][%w_%.]*)")
      tag.name = enum_name
