...
---@type table<string, boolean>
local OPS1 = {}
for char in ("+-*/%^#&~|<>=(){}[];:,."):gmatch(".") do OPS1[char] = true end
...
local function long_bracket_level(state, pos)
  if byte(state.src, pos) ~= B_LBRACKET then return nil end
  local probe = pos + 1
  while byte(state.src, probe) == B_EQ do probe = probe + 1 end
  if byte(state.src, probe) ~= B_LBRACKET then return nil end
...
local function read_long_body(state, level, what)
  ...
  if b == B_NEWLINE or b == B_RETURN then newline(state) end
...
local function read_quoted(state)
  ...
  while true do
    ...
    if b == nil then fail(state, "unfinished string") end
    if b == quote then
      state.pos = state.pos + 1
      break
    ...
    elseif b == B_BACKSLASH then
      ...
      if e == nil then fail(state, "unfinished string") end
      local simple = SIMPLE_ESCAPES[e]
      if simple then
        n = n + 1; pieces[n] = simple
        state.pos = state.pos + 1
      ...
      elseif e == 120 or e == 88 then -- \xXX
        ...
        if not hex:match("^%x%x$") then fail(state, "hexadecimal digit expected") end
        n = n + 1; pieces[n] = string.char(tonumber(hex, 16))
      ...
      elseif e == 117 then -- \u{XXX}
        ...
        if not closing then fail(state, "missing '}' in \\u{xxxx}") end
      ...
      elseif is_digit(e) then -- \ddd
        ...
        if value > 255 then fail(state, "decimal escape too large") end
        n = n + 1; pieces[n] = string.char(value)
      else
      ...
      n = n + 1; pieces[n] = sub(src, state.pos, run - 1)
      state.pos = run
    end
  end
...
local function read_number(state)
  ...
  if is_hex_literal then exp_lower, exp_upper = 112, 80 end -- p, P
  ...
  while true do
    ...
    if b == exp_lower or b == exp_upper then
      ...
      if sign == 43 or sign == B_DASH then state.pos = state.pos + 1 end
...
function M.lex(src)
  ...
  local ok, err = pcall(function()
    ...
    while state.pos <= state.len do
      ...
      elseif b == B_DASH and byte(src, state.pos + 1) == B_DASH then
        ...
        if level then
          ...
          cn = cn + 1
          comments[cn] = {
            text = text, doc = false, long = true,
            l = start_line, c = start_col,
            el = state.line, ec = column(state, state.pos),
          }
        else
          ...
          comments[cn] = {
            ...
            doc = is_doc, long = false,
            l = start_line, c = start_col,
            el = start_line, ec = column(state, text_end + 1),
          }
      ...
      elseif is_name_start(b) then
        ...
        repeat state.pos = state.pos + 1 until not is_name_part(byte(src, state.pos))
        local word = sub(src, start, state.pos - 1)
        tn = tn + 1
        tokens[tn] = {
          t = KEYWORDS[word] and "keyword" or "name",
          v = word,
          l = state.line, c = start_col,
          el = state.line, ec = column(state, state.pos),
        }
      ...
      elseif b == B_LBRACKET and long_bracket_level(state, state.pos) then
        ...
        ---@type string, integer
        local op, width
        if OPS3[three] then op, width = three, 3
        elseif OPS2[two] then op, width = two, 2
        elseif OPS1[one] then op, width = one, 1
        else fail(state, "unexpected symbol near '" .. one .. "'") end
        ...
        tokens[tn] = {
          t = "op", v = op,
          l = state.line, c = start_col,
          el = state.line, ec = start_col + width,
        }
      end
    end
    ...
    tokens[tn] = {
      t = "eof", v = "<eof>",
      l = state.line, c = column(state, state.pos),
      el = state.line, ec = column(state, state.pos),
    }
  end)
