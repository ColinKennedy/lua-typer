...
function M.run(model, ctx)
  if not ctx.config.undefined_globals then return end
  ...
  for _, read in ipairs(model.global_reads) do
    ...
    if not reported[name]
      and not model.globals[name]
      and not registry.globals[name]
      and not ignore[name]
    then
      -- Report once per name per file; a typo repeated ten times is one defect.
      reported[name] = true
      ctx.emit(diagnostic.new(model.path, read, "undefined-global",
