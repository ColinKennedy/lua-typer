...
local function parse_codes(text)
    ...
    local codes = {}
    for code in trimmed:gmatch("[%w%-]+") do
        codes[code] = true
    end
                ...
                if file_rest then
                    ...
                    if rest then
                        ...
                        if on_line and model.docs.code_lines[comment.l] then
                            ...
                            result.ranges[#result.ranges + 1] = { from = on_line.l, to = on_line.el, codes = codes }
                        else
                            local target = statement_after(statements, comment.l)
                            if target then
                                result.ranges[#result.ranges + 1] = { from = target.l, to = target.el, codes = codes }
                            else
                                -- A trailing comment with no statement after it suppresses its
                                -- own line, which is the only sensible reading.
                                result.ranges[#result.ranges + 1] = { from = comment.l, to = comment.l, codes = codes }
                            end
                        end
                    end
                end
