...
local function describe(node)
  ...
  if node.k == "Index" then return describe(node.obj) .. "[...]" end
  return "?"
end
...
local function classify(node)
  if not node then return "other" end
  local kind = node.k
  ...
  if kind == "Call" and node.callee and node.callee.k == "Name"
    and node.callee.name == "require" then
    return "require"
  end
  return "other"
end
...
local function attach_tags(binding, tags)
  ...
  for _, tag in ipairs(tags) do
    if tag.kind == "class" then binding.class_tag = tag end
    if tag.kind == "enum" then binding.enum_tag = tag end
    if tag.kind == "type" and not binding.type_tag then binding.type_tag = tag end
  end
end
...
local function enter_function(walker, node, tags, display, anchor, owner, exempt)
  ...
  local info = {
    ...
    tags = tags or {},
    display = display,
    l = anchor.l, c = anchor.c, ec = anchor.ec or anchor.c,
  ...
  for _, param in ipairs(node.params) do
    declare(walker, param.name, {
      name = param.name, l = param.l, c = param.c, ec = param.ec,
      scope = "local", kind = "other", signals = {}, fields = {}, methods = {},
      tags = {},
...
walk_expr = function(walker, node, position, tags)
    ...
    if not binding then
      ...
      if not model.globals[node.name] then
        model.global_reads[#model.global_reads + 1] = {
          name = node.name, l = node.l, c = node.c, ec = node.ec,
        }
      end
    end
  ...
  elseif kind == "Index" then
    ...
    if node.computed then walk_expr(walker, node.key) end
  ...
  elseif kind == "Function" then
    if position == "inline" then
      ...
      enter_function(walker, node, tags or {}, walker.pending_name or "<anonymous>",
        node, nil, false)
    end

  elseif kind == "Table" then
    for _, field in ipairs(node.fields) do
      if field.computed then walk_expr(walker, field.key) end
...
local function record_field_assignment(walker, target, value, tags)
  if target.k ~= "Index" or target.computed then return end
  ...
  for _, tag in ipairs(tags or {}) do
    if tag.kind == "type" then annotated = true break end
  end
...
walk_stat = function(walker, node)
  ...
  if kind == "Local" then
    ...
    for index, name_info in ipairs(node.names) do
      ...
      local binding = {
        name = name_info.name,
        l = name_info.l, c = name_info.c, ec = name_info.ec,
        scope = "local",
        kind = init_kind,
        decl = node,
        init = init,
      ...
      if init and init.k == "Call" and init.callee and init.callee.k == "Name"
        and init.callee.name == "setmetatable" and init.args and init.args[2] then
  ...
  elseif kind == "LocalFunction" then
    ...
    ---@type typer.Binding
    local binding = {
      name = node.name, l = node.name_l, c = node.name_c, ec = node.name_ec,
      scope = "local", kind = "function", decl = node, init = node.fn,
      signals = {}, fields = {}, methods = {}, tags = {},
    }
    attach_tags(binding, tags)
    model.bindings[#model.bindings + 1] = binding
  ...
  elseif kind == "FunctionStat" then
    ...
    if target.k == "Index" and not target.computed then
      ...
      if owner then
        ...
        owner.methods[target.name] = {
          name = target.name,
          l = target.key_l or target.l, c = target.key_c or target.c,
          ec = target.key_ec or target.ec,
          is_function = true,
        }
      end
    ...
    elseif target.k == "Name" then
      ...
      if not existing and not model.globals[target.name] then
        ---@type typer.Binding
        local binding = {
          name = target.name, l = target.l, c = target.c, ec = target.ec,
          scope = "global", kind = "function", decl = node, init = node.fn,
          signals = {}, fields = {}, methods = {}, tags = {},
        }
        attach_tags(binding, tags)
    ...
    for index, target in ipairs(node.targets) do
      ...
      if target.k == "Name" then
        ...
        if existing then
          ...
          if global then
            if value then global.reassigned = true end
          else
            ---@type typer.Binding
            local binding = {
              name = target.name, l = target.l, c = target.c, ec = target.ec,
              scope = "global", kind = classify(value), decl = node, init = value,
              signals = {}, fields = {}, methods = {}, tags = {},
  ...
  elseif kind == "Return" then
    ...
    if exprs and #exprs > 0 then
      if current then
        ...
        if #exprs > current.return_arity then current.return_arity = #exprs end
        ...
        for _, expr in ipairs(exprs) do
          if expr.k == "Nil" then current.has_nil_return = true end
        end
      else
  ...
  elseif kind == "ForNum" then
    ...
    declare(walker, node.var.name, {
      name = node.var.name, l = node.var.l, c = node.var.c, ec = node.var.ec,
      scope = "local", kind = "scalar", signals = {}, fields = {}, methods = {}, tags = {},
      loop_var = true,
    })
    walk_block(walker, node.body)
    pop_scope(walker)
  ...
  elseif kind == "ForIn" then
    for _, expr in ipairs(node.exprs) do walk_expr(walker, expr) end
    push_scope(walker)
    for _, name_info in ipairs(node.names) do
      declare(walker, name_info.name, {
        name = name_info.name, l = name_info.l, c = name_info.c, ec = name_info.ec,
        scope = "local", kind = "other", signals = {}, fields = {}, methods = {}, tags = {},
        loop_var = true,
      })
    end
    walk_block(walker, node.body)
...
function M.class_shape_reason(binding)
  for _, signal in ipairs(M.CLASS_SIGNALS) do
    if binding.signals[signal] then return M.CLASS_SIGNAL_REASON[signal] end
  end
  return nil
end
