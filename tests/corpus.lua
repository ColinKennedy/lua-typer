...
---@type string[]
local files = {}
for _, root in ipairs(roots) do collect(root, files) end
table.sort(files)
...
for _, entry in ipairs(sources) do lexer.lex(entry.src) end
    ...
    if loaded then
      ...
      if #failure_lines < 25 then
        failure_lines[#failure_lines + 1] =
          ("%s:%d:%d: %s"):format(entry.path, err.l, err.c, err.msg)
      end
    else
      skipped = skipped + 1
    end
...
print(("files          : %d (%d unparseable by host %s, ignored)")
  :format(#sources, skipped, compat.lua_version))
...
for _, line in ipairs(failure_lines) do print("  " .. line) end
