...
local function decode_string(decoder)
  ...
  while true do
    ...
    if char == "" then error("unterminated string in JSON", 0) end
    if char == '"' then
      decoder.pos = decoder.pos + 1
      break
    elseif char == "\\" then
...
decode_value = function(decoder)
  ...
  if char == "{" then
    ...
    while true do
      ...
      if delimiter ~= "," then error("',' expected in JSON object", 0) end
    end
  ...
  elseif char == "[" then
    ...
    while true do
      ...
      if delimiter ~= "," then error("',' expected in JSON array", 0) end
    end
