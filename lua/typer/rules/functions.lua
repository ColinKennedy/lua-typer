  ...
  for _, info in ipairs(model.functions) do
    if not typed_by_context(info) then
      ...
      for _, tag in ipairs(param_tags) do
        if tag.name then by_name[tag.name] = tag end
      end
      ...
      for _, param in ipairs(node.params) do
        ...
        if not param.implicit then real_params[#real_params + 1] = param end
      end
      ...
      if node.is_method and by_name["self"] then
        ...
        for _, candidate in ipairs(param_tags) do
          if candidate.name ~= "self" then without_self[#without_self + 1] = candidate end
        end
      ...
      for index, param in ipairs(real_params) do
        ...
        if not tag then
          if param.vararg then
            ctx.emit(diagnostic.new(model.path, param, "missing-vararg",
              "function declares '...' but has no ---@param ... annotation",
              "---@param ... <type>"))
          else
            ctx.emit(diagnostic.new(model.path, param, "missing-param",
          ...
          if positional and positional.name and positional.name ~= param.name
            and not by_name[positional.name .. "\0"] then
            local matches_some = false
            for _, other in ipairs(real_params) do
              if other.name == positional.name then matches_some = true end
            end
            if not matches_some then
              ctx.emit(diagnostic.new(model.path,
                { l = positional.l, c = positional.name_col or positional.c,
                  ec = positional.name_ec },
                "param-name-mismatch",
                ("---@param '%s' does not match parameter '%s'")
                  :format(positional.name, param.name),
                ("---@param %s <type>"):format(param.name)))
            end
          end
          ...
          if config.optional_param ~= "off" and param.defaulted and not tag.optional then
            ctx.emit(diagnostic.new(model.path,
              { l = tag.l, c = tag.name_col or tag.c, ec = tag.name_ec },
              "optional-param",
              ("parameter '%s' is defaulted in the body but not marked optional")
                :format(param.name),
              ("---@param %s? <type>"):format(param.name)))
          end
        end
      ...
      if #param_tags > #real_params and #overloads == 0 then
        ...
        local real_names = {}
        for _, param in ipairs(real_params) do real_names[param.name] = true end
        for _, tag in ipairs(param_tags) do
      ...
      if info.has_value_return then
        if #return_tags < info.return_arity then
          ...
          ctx.emit(diagnostic.new(model.path, info, "missing-return",
            ("function returns %d value%s but declares %d ---@return; %d missing")
              :format(info.return_arity, info.return_arity == 1 and "" or "s",
                      #return_tags, missing),
            "---@return <type>"))
        elseif #return_tags > info.return_arity and #overloads == 0
          and not info.indeterminate_arity then
          local extra = return_tags[info.return_arity + 1]
          ctx.emit(diagnostic.new(model.path, { l = extra.l, c = extra.c },
            "return-arity-mismatch",
            ("%d ---@return annotations but the function returns at most %d value%s")
              :format(#return_tags, info.return_arity,
                      info.return_arity == 1 and "" or "s"),
            nil))
        end

        if config.optional_return ~= "off" and info.has_nil_return
          and return_tags[1] and return_tags[1].type then
          local first = return_tags[1].type
          local optional = first.k == "optional"
            or (first.k == "union" and (function()
                  for _, part in ipairs(first.parts) do
                    if part.k == "name" and part.name == "nil" then return true end
                  end
                  return false
                end)())
