...
function M.signature(path)
  if not M.has_lfs then return nil end
  ...
  if M.has_lfs then
    ...
    if pipe then
      for line in pipe:lines() do
        if line ~= "" then out[#out + 1] = compat.normalize(line) end
      end
      pipe:close()
    end
  end
...
function M.mkdir_p(path)
  if M.has_lfs then
    ...
    for piece in path:gmatch("[^/]+") do
      ...
      if not M.is_dir(accumulated) then lfs.mkdir(accumulated) end
    end
    return M.is_dir(path)
  end

  local command = compat.is_windows
    and ('mkdir "%s" 2>nul'):format(path:gsub("/", "\\"))
...
---@return string
function M.cwd()
  if M.has_lfs then return compat.normalize(lfs.currentdir()) end
