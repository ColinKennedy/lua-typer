...
function M.build(chunk)
  ...
  for _, token in ipairs(chunk.tokens) do
    if token.t ~= "eof" then
      for line = token.l, token.el do code_lines[line] = true end
    end
  end
  ...
  for _, comment in ipairs(chunk.comments) do
    if comment.doc and not comment.long then
      if code_lines[comment.l] then
        ...
        ---@type typer.DocBlock
        local block = {
          comments = { comment }, l = comment.l, el = comment.l,
          trailing = true, consumed = false,
        }
...
function M.tags_for(index, node)
  ...
  if leading then
    ...
    for _, tag in ipairs(leading.tags) do out[#out + 1] = tag end
  end

  local trailing = index.trailing_by_line[node.el or node.l]
  if trailing then
    trailing.consumed = true
    for _, tag in ipairs(trailing.tags) do out[#out + 1] = tag end
  end
...
function M.find(tags, kind)
  for _, tag in ipairs(tags) do
    if tag.kind == kind then return tag end
  end
  return nil
end
...
function M.find_all(tags, kind)
  ...
  local out = {}
  for _, tag in ipairs(tags) do
    if tag.kind == kind then out[#out + 1] = tag end
  end
  return out
end
