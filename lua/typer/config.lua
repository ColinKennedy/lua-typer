...
local function load_lua_config(path)
  ...
  local sandbox = {
    ...
    table = table, math = math, select = select,
  }
...
function M.discover(start)
  ...
  while guard < 64 do
    ...
    for _, name in ipairs({ ".typer.lua", ".typer.json" }) do
      ...
      if compat.file_exists(candidate) then return candidate end
    end
...
local function merge(target, source)
  for key, value in pairs(source) do
    if key == "severity" and type(value) == "table" then
      ...
      for code, level in pairs(value) do target.severity[code] = level end
    else
      target[key] = value
    end
  end
...
function M.load(explicit_path, cwd)
  ...
  if path then
    ...
    if not loaded then return config, err end
    merge(config, loaded)
  ...
  for _, key in ipairs({ "source_roots", "stub_paths", "lua_path", "exclude", "ignore_missing" }) do
    if type(config[key]) == "string" then config[key] = { config[key] } end
...
function M.ignores_module(config, module)
  for _, pattern in ipairs(config.ignore_missing_patterns or {}) do
    if module:match(pattern) then return true end
  end
  return false
end
