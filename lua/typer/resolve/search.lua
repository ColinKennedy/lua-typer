...
local function bundled_stub_root()
  ...
  for template in package.path:gmatch("[^;]+") do
    ...
    if compat.file_exists(candidate) then
      ...
      if fs.is_dir(root) then return root end
    end
  end
  return nil
end
...
local function add_luarc(root, out)
  ...
  if not compat.file_exists(path) then return end

  local decoded = json.decode(compat.read_file(path) or "")
  if type(decoded) ~= "table" then return end

  local library = decoded["workspace.library"]
    or (type(decoded.workspace) == "table" and decoded.workspace.library)
...
local function version_env_names(config)
  ...
  if version == "jit" then return { "LUA_PATH", "LUA_PATH_5_1" } end
...
function M.build(config, overrides)
  ...
  if stub_root then
    local version_dir = compat.join(stub_root, config.lua_version == "jit" and "5.1"
      or (config.lua_version or "5.1"))
  ...
  for _, dir in ipairs(config.source_roots or {}) do
    ...
    if fs.is_dir(resolved) then add_dir(resolved, "workspace", entries) end
  end
